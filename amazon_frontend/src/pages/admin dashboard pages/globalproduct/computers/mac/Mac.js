...
const Mac = () => {
  ...
  useEffect(() => {
    const mac = async () => {
      try {
        const { data } = await axios.get("/api/computers/mac/all", {
          headers: {
  ...
  useEffect(() => {
    ...
    socket.on("new macs", (macDetails) => {
      setAllData((pre) => [...pre, macDetails]);
    });
