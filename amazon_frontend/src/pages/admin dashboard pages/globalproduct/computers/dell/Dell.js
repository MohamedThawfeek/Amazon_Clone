...
import style from "./Dell.module.css";
...
const Dell = () => {
  ...
  useEffect(() => {
    const dell = async () => {
      try {
        const { data } = await axios.get("/api/computers/dell/all", {
          headers: {
  ...
  useEffect(() => {
    ...
    socket.on("new dells", (dellDetails) => {
      setAllData((pre) => [...pre, dellDetails]);
    });
    return () => {
      socket.disconnect();
    };
