...
import style from "./Hp.module.css";
...
const Hp = () => {
  ...
  useEffect(() => {
    const hp = async () => {
      try {
        const { data } = await axios.get("/api/computers/hp/all", {
          headers: {
  ...
  useEffect(() => {
    ...
    socket.on("new hps", (hpDetails) => {
      setAllData((pre) => [...pre, hpDetails]);
    });
