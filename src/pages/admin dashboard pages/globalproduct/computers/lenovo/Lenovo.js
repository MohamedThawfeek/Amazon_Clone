...
import style from "./Lenovo.module.css";
...
const Lenovo = () => {
  ...
  useEffect(() => {
    const lenovo = async () => {
      try {
        const { data } = await axios.get("/api/computers/lenovo/all", {
          headers: {
  ...
  useEffect(() => {
    ...
    socket.on("new lenovos", (lenovoDetails) => {
      setAllData((pre) => [...pre, lenovoDetails]);
    });
    return () => {
      socket.disconnect();
    };
  ...
  const del = async (id) => {
    try {
      const { data } = await axios.delete(
        `/api/computers/lenovo/delete/${id}`,
  ...
  const edit = async () => {
    try {
      const { data } = await axios.put(
        `/api/computers/lenovo/update/${id}`,
        input,
