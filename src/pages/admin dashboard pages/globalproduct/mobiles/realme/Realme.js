...
import style from "./Realme.module.css";
...
const Realme = () => {
  ...
  useEffect(() => {
    const realme = async () => {
      try {
        const { data } = await axios.get("/api/mobiles/realme/all", {
          headers: {
  ...
  useEffect(() => {
    socket.connect();
    socket.on("new realme", (realmeDetails) => {
      setAllData((pre) => [...pre, realmeDetails]);
    });
    return () => {
      socket.disconnect();
    };
