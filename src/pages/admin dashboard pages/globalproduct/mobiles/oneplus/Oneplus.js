...
const Oneplus = () => {
  ...
  useEffect(() => {
    socket.connect();
    socket.on("new oneplus", (oneplusDetails) => {
      setAllData((pre) => [...pre, oneplusDetails]);
    });
    return () => {
      socket.disconnect();
    };
