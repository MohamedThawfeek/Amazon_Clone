...
const Samsung = () => {
  ...
  useEffect(() => {
    socket.connect();
    socket.on("new samsung", (samsungDetails) => {
      setAllData((pre) => [...pre, samsungDetails]);
    });
    return () => {
      socket.disconnect();
    };
