...
import style from "./Poco.module.css";
...
const Poco = () => {
  ...
  useEffect(() => {
    const poco = async () => {
      try {
        const { data } = await axios.get("/api/mobiles/poco/all", {
          headers: {
  ...
  useEffect(() => {
    socket.connect();
    socket.on("new poco", (pocoDetails) => {
      setAllData((pre) => [...pre, pocoDetails]);
    });
    return () => {
      socket.disconnect();
    };
