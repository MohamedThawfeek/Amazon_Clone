...
const Iphone = () => {
  ...
  useEffect(() => {
    const iphone = async () => {
      try {
        const { data } = await axios.get("/api/mobiles/iphone/all", {
          headers: {
  ...
  useEffect(() => {
    socket.connect();
    socket.on("new iphones", (iphoneDetails) => {
      setAllData((pre) => [...pre, iphoneDetails]);
    });
    return () => {
      socket.disconnect();
    };
