...
const Tv = () => {
  ...
  useEffect(() => {
    const tv = async () => {
      try {
        const { data } = await axios.get("/api/homeAppliance/tv/all", {
          headers: {
  ...
  useEffect(() => {
    socket.connect();
    socket.on("new tv", (tvDetails) => {
      setAllData((pre) => [...pre, tvDetails]);
    });
    return () => {
      socket.disconnect();
    };
  ...
  const del = async (id) => {
    try {
      const { data } = await axios.delete(
        `/api/homeAppliance/tv/delete/${id}`,
  ...
  const edit = async () => {
    try {
      const { data } = await axios.put(
        `/api/homeAppliance/tv/update/${id}`,
        input,
