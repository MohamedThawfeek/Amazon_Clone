...
const Oven = () => {
  ...
  useEffect(() => {
    socket.connect();
    socket.on("new ovens", (ovenDetails) => {
      setAllData((pre) => [...pre, ovenDetails]);
    });
    return () => {
      socket.disconnect();
    };
  ...
  const del = async (id) => {
    try {
      const { data } = await axios.delete(
        `/api/homeAppliance/oven/delete/${id}`,
  ...
  const edit = async () => {
    try {
      const { data } = await axios.put(
        `/api/homeAppliance/oven/update/${id}`,
        input,
