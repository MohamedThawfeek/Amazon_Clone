...
const Fan = () => {
  ...
  useEffect(() => {
    const fan = async () => {
      try {
        const { data } = await axios.get("/api/homeAppliance/fan/all", {
          headers: {
  ...
  useEffect(() => {
    socket.connect();
    socket.on("new fan", (fanDetails) => {
      setAllData((pre) => [...pre, fanDetails]);
    });
    return () => {
      socket.disconnect();
    };
  ...
  const del = async (id) => {
    try {
      const { data } = await axios.delete(
        `/api/homeAppliance/fan/delete/${id}`,
  ...
  const edit = async () => {
    try {
      const { data } = await axios.put(
        `/api/homeAppliance/fan/update/${id}`,
        input,
