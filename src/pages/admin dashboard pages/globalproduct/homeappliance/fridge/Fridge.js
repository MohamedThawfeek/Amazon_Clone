...
const Fridge = () => {
  ...
  useEffect(() => {
    socket.connect();
    socket.on("new fridge", (fridgeDetails) => {
      setAllData((pre) => [...pre, fridgeDetails]);
    });
    return () => {
      socket.disconnect();
    };
  ...
  const del = async (id) => {
    try {
      const { data } = await axios.delete(
        `/api/homeAppliance/fridge/delete/${id}`,
  ...
  const edit = async () => {
    try {
      const { data } = await axios.put(
        `/api/homeAppliance/fridge/update/${id}`,
        input,
