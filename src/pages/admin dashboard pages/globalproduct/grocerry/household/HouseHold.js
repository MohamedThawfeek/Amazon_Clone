...
import style from "./HouseHold.module.css";
...
const HouseHold = () => {
  ...
  useEffect(() => {
    const houseHold = async () => {
      try {
        const { data } = await axios.get("/api/grocery/household/all", {
          headers: {
  ...
  useEffect(() => {
    ...
    socket.on("new household_care", (household_careDetails) => {
      setAllData((pre) => [...pre, household_careDetails]);
    });
    return () => {
      socket.disconnect();
    };
  ...
  const del = async (id) => {
    try {
      const { data } = await axios.delete(
        `/api/grocery/household/delete/${id}`,
  ...
  const edit = async () => {
    try {
      const { data } = await axios.put(
        `/api/grocery/household/update/${id}`,
        input,
          ...
          <TableBody>
            {dataSearch.map((users) => (
              ...
              >
                ...
                <TableCell align="center">{users.weight}</TableCell>
