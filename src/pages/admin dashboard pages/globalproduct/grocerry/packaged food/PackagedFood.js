...
const PackagedFood = () => {
  ...
  useEffect(() => {
    ...
    socket.on("new packaged_foods", (packaged_foodDetails) => {
      setAllData((pre) => [...pre, packaged_foodDetails]);
    });
    return () => {
      socket.disconnect();
    };
  ...
  const del = async (id) => {
    try {
      const { data } = await axios.delete(
        `/api/grocery/packagedfood/delete/${id}`,
  ...
  const edit = async () => {
    try {
      const { data } = await axios.put(
        `/api/grocery/packagedfood/update/${id}`,
        input,
