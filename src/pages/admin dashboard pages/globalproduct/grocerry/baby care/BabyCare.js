...
const BabyCare = () => {
  ...
  useEffect(() => {
    ...
    socket.on("new babycares", (babycaresDetails) => {
      setAllData((pre) => [...pre, babycaresDetails]);
    });
    return () => {
      socket.disconnect();
    };
  ...
  const del = async (id) => {
    try {
      const { data } = await axios.delete(
        `/api/grocery/babycare/delete/${id}`,
  ...
  const edit = async () => {
    try {
      const { data } = await axios.put(
        `/api/grocery/babycare/update/${id}`,
        input,
