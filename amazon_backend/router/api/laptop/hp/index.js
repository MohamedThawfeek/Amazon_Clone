...
router.get("/all", async (req, res) => {
  try {
    const product = await Hp.find({}).select(
...
router.get("/:id", async (req, res) => {
  try {
    const product = await Hp.find({ _id: req.params.id }).select(
...
router.put("/update/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Hp.findByIdAndUpdate(
      {
        _id: req.params.id,
      },
      {
...
router.delete("/delete/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Hp.findByIdAndDelete(
      {
        _id: req.params.id,
      },
