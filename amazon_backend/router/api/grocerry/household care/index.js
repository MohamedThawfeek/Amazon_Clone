...
const Household_Care = require("../../../../models/grocerry/household care/household_Care");
const { authenticate, permit } = require("../../../../middleware");

router.get("/", (req, res) => {
  res.send("Household_Care");
});

router.post("/add", [authenticate, permit([1])], async (req, res) => {
  try {
    const household_Care = await Household_Care.create(req.body);
...
router.get("/all", async (req, res) => {
  try {
    const product = await Household_Care.find({}).select(
...
router.get("/:id", async (req, res) => {
  try {
    const product = await Household_Care.find({ _id: req.params.id }).select(
      "-createdAt -updatedAt -password -__v"
    );
...
//   try {
//     const household_Care = await Household_Care.find({})
//       .select("-createdAt -updatedAt -password -__v")
//       .sort({ price: 1 });

//     res.json({ success: true, household_Care });
//   } catch (error) {
//     res.json({ msg: error.message });
//   }
// });
...
router.put("/update/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Household_Care.findByIdAndUpdate(
      {
        _id: req.params.id,
      },
      {
...
router.delete("/delete/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Household_Care.findByIdAndDelete(
      {
        _id: req.params.id,
      },
