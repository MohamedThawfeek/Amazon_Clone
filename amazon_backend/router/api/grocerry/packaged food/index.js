...
const Packaged_Food = require("../../../../models/grocerry/packaged food/packaged_food");
const { authenticate, permit } = require("../../../../middleware");

router.get("/", (req, res) => {
  res.send("Packaged_Food");
});

router.post("/add", [authenticate, permit([1])], async (req, res) => {
  try {
    const packaged_Food = await Packaged_Food.create(req.body);
...
router.get("/all", async (req, res) => {
  try {
    const product = await Packaged_Food.find({}).select(
...
router.get("/:id", async (req, res) => {
  try {
    const product = await Packaged_Food.find({ _id: req.params.id }).select(
      "-createdAt -updatedAt -password -__v"
    );
...
//   try {
//     const packaged_Food = await Packaged_Food.find({})
//       .select("-createdAt -updatedAt -password -__v")
//       .sort({ price: 1 });

//     res.json({ success: true, packaged_Food });
//   } catch (error) {
//     res.json({ msg: error.message });
//   }
// });
...
router.put("/update/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Packaged_Food.findByIdAndUpdate(
      {
        _id: req.params.id,
      },
      {
...
router.delete("/delete/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Packaged_Food.findByIdAndDelete(
      {
        _id: req.params.id,
      },
