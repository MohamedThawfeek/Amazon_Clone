...
router.get("/all", async (req, res) => {
  try {
    const product = await Iphone.find({}).select(
...
router.get("/all/reviews", async (req, res) => {
  try {
    const product = await Iphone.find({})
      .select(
...
router.get("/:id", async (req, res) => {
  try {
    const product = await Iphone.find({ _id: req.params.id }).select(
...
//   try {
//     const iphone = await Iphone.find({})
//       .select("-createdAt -updatedAt -password -__v")
//       .sort({ price: -1 });

//     res.json({ success: true, iphone });
//   } catch (error) {
//     res.json({ msg: error.message });
//   }
// });
...
router.delete("/delete/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Iphone.findByIdAndDelete(
      {
        _id: req.params.id,
      },
