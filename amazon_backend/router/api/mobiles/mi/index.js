...
router.post("/add", [authenticate, permit([1])], async (req, res) => {
  try {
    const mi = await Mi.create(req.body);
...
router.get("/all", async (req, res) => {
  try {
    const product = await Mi.find({}).select(
...
router.get("/:id", async (req, res) => {
  try {
    const product = await Mi.find({ _id: req.params.id }).select(
...
//   try {
//     const mi = await Mi.find({})
//       .select("-createdAt -updatedAt -password -__v")
//       .sort({ price: 1 });

//     res.json({ success: true, mi });
//   } catch (error) {
//     res.json({ msg: error.message });
//   }
// });
...
router.put("/update/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Mi.findByIdAndUpdate(
      {
        _id: req.params.id,
      },
      {
...
router.delete("/delete/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Mi.findByIdAndDelete(
      {
        _id: req.params.id,
      },
