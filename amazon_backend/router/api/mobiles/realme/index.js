...
router.post("/add", [authenticate, permit([1])], async (req, res) => {
  try {
    const realme = await Realme.create(req.body);
...
router.get("/all", async (req, res) => {
  try {
    const product = await Realme.find({}).select(
...
router.get("/all/reviews", async (req, res) => {
  try {
    const product = await Realme.find({})
      .select(
...
router.get("/:id", async (req, res) => {
  try {
    const product = await Realme.find({ _id: req.params.id }).select(
...
//   try {
//     const realme = await Realme.find({})
//       .select("-createdAt -updatedAt -password -__v")
//       .sort({ price: 1 });

//     res.json({ success: true, realme });
//   } catch (error) {
//     res.json({ msg: error.message });
//   }
// });
...
router.put("/update/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Realme.findByIdAndUpdate(
      {
        _id: req.params.id,
      },
      {
...
router.delete("/delete/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Realme.findByIdAndDelete(
      {
        _id: req.params.id,
      },
