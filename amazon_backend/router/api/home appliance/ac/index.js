...
router.get("/all", async (req, res) => {
  try {
    const product = await AC.find({}).select(
...
router.get("/:id", async (req, res) => {
  try {
    const product = await AC.find({ _id: req.params.id }).select(
...
router.put("/update/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await AC.findByIdAndUpdate(
      {
        _id: req.params.id,
      },
      {
...
router.delete("/delete/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await AC.findByIdAndDelete(
      {
        _id: req.params.id,
      },
