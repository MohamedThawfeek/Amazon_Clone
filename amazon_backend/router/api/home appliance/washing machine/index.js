...
router.post("/add", [authenticate, permit([1])], async (req, res) => {
  try {
    const washing_machines = await Washing_Machines.create(req.body);
...
router.get("/all", async (req, res) => {
  try {
    const product = await Washing_Machines.find({}).select(
...
router.get("/all/reviews", async (req, res) => {
  try {
    const product = await Washing_Machines.find({})
      .select(
...
router.get("/:id", async (req, res) => {
  try {
    const product = await Washing_Machines.find({ _id: req.params.id }).select(
...
//   try {
//     const washing_machines = await Washing_Machines.find({})
//       .select("-createdAt -updatedAt -password -__v")
//       .sort({ price: 1 });

//     res.json({ success: true, washing_machines });
//   } catch (error) {
//     res.json({ msg: error.message });
//   }
// });
...
router.put("/update/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Washing_Machines.findByIdAndUpdate(
      {
        _id: req.params.id,
      },
      {
...
router.delete("/delete/:id", [authenticate, permit([1])], async (req, res) => {
  try {
    const product = await Washing_Machines.findByIdAndDelete(
      {
        _id: req.params.id,
      },
